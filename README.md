    ...
    steps:
      ...
      - name: Setup Perl
        ...
        with:
          perl-version: "5.38"
      - name: Run Perl Lint
        run: bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/perl-lint/master/src/pipeline.sh)
```
...
```bash
bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/perl-lint/master/src/pipeline.sh)
```
